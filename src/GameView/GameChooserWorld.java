...
public class GameChooserWorld extends TitleWorld implements InteractionBox {
	...
	public void paintDisplay(Graphics2D g, int xSize, int ySize, int xOffset,
		...
		else if ((matrix2.getSelectedNodeLocation()[1] == 0) && (matrix2.getSelectedNodeLocation()[0] == 1)){
			
			expandImage(g, "ImageFiles/PokemonTitle.png", 1, 0, increaseX, increaseY);
		}
	...
	public void setGameString() {
		isExpanding = true;
		System.out.println(isExpanding);
		if (increaseY > 500) {
			myGameString = matrix2.getCurrentNode().getGame();
		}
	}

	private void expandImage(Graphics g2d, String imageFile, int xPos,
			int yPos, int increaseX, int increaseY) {
		if ((xPos == 1) && (yPos == 0)) {
			System.out.println("expanding");
			Image img = new ScaledImage(250 + increaseX, 250 + increaseY,
					imageFile).scaleImage();
			g2d.drawImage(img, 250-increaseX, 0, null);
		}
		// else if((xPos == 0) && (yPos == 1)){
		// Image img = new ScaledImage(250 + increaseX, 250 + increaseY,
		// "ImageFiles/PokemonTitle.png").scaleImage();
