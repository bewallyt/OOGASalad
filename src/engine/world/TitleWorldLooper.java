...
public class TitleWorldLooper extends GameLooper {
	...
	public TitleWorldLooper(TitleWorld currentWorld) {
		...
		myWorld.getPlayer().setState(new TitleState(myWorld.getPlayer(), myTM));
	...
	public World doLoop() {
		...
		if (isGameLoaded) {
			...
			p.setPosition(gm.getXStart() * Constants.TILE_SIZE, gm.getYStart()
					* Constants.TILE_SIZE);
			return gm.getInitialWorld();

		}

		if (myWorld.getPlayer().getState() instanceof ExitState) {
